...
/datum/toolgun_mode/spawn_mode/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	switch(action)
		if("browse_to")                          // ← Новый экшен — переход по папкам
			var/path_to_browse = trimtext(params["path"])
			if(length(path_to_browse) && (path_to_browse == "/obj" || text2path(path_to_browse)))
...
/datum/toolgun_mode/build_mode/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	switch(action)
		if("browse_to")
			var/path_to_browse = trimtext(params["path"])
			if(length(path_to_browse) && (path_to_browse == "/turf" || text2path(path_to_browse)))
				current_browse_path = path_to_browse
				loaded_limit = page_size
			return TRUE
